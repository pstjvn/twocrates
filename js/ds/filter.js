...
/**
 * Global flag set per wall. If this is the last wall no corner items
 * should be alloed on it.
 * @private
 * @type {boolean}
 */
_.isLastWall_ = false;
 ...
 * @param {boolean=} opt_use_second_width Flag if the item is corner item and is
 *   the first in the row.
 * @return {function(pstj.ds.ListItem): boolean} The produced filter
 *   function.
 */
_.createChangeFilter = function(
    category, availablewidth, opt_use_second_width) {
  console.log(category, availablewidth, opt_use_second_width);
  return function(item) {
    ...
    if (item.getProp(
        (!!opt_use_second_width) ?
            Struct.WIDTH2 :
            Struct.WIDTH) > availablewidth) {
 ...
 * @param {boolean=} opt_use_second_width If the secondary width of the items
 *   should be used.
 * @return {function(pstj.ds.ListItem): boolean} The produced filter
 *   function.
 */
_.createModelFilter = function(model, availablewidth, opt_use_second_width) {
  return function(item) {
