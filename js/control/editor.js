 ...
 *   the datasheet component and depends on its implementation for determining
...
k3d.control.Editor = function() {
   ...
   *   user agent resizes and thus allow us to fit the sheet correctly even on
   *   orientation changes.
   * @type {k3d.ui.Filler}
   */
  ...
  /**
   * Reference to the kitchen project data structure.
   * @type {k3d.ds.KitchenProject}
   * @protected
   */
  this.data = null;
  /**
   * Reference to the currently displayed wall.
   * @type {k3d.ds.Wall}
   * @protected
   */
   ...
   *   millimeter values an match them to pixel value.
   * @type {number}
   * @private
   */
  this.scaleFactor_ = 1;
  ...
  /**
   * The list of lists of stop points for the movement.
   * @type {Array.<number>}
   * @protected
   */
  ...
  /**
   * The offsets coming from two row items.
  ...
  /**
   * Bound and delayed function for checking for overflows when adding corner
   *   items.
   * @type {goog.async.Delay}
   * @private
   */
...
/**
 * Loads the kitchen project data record.
...
_.initialize = function() {
  ...
  k3d.component.PubSub.subscribe(k3d.component.PubSubTopic, function() {
    ...
    if (currentoffset < 0) {
      ...
      // Determine the start (edge) point against which to calculate where the
      ...
      for (var i = index_of_child - 1; i >= 0; i--) {
        ...
        if ((startpoint + distancetraveled) < this.stopPoints[i]) {
          ...
          // If the item is not passed return it of its normal position.
          pstj.lab.style.css.setTranslation(rowOfChildren[i].getElement(), 0,
              0);
        }
      }
    } else if (currentoffset > 0) {
      var startpoint = this.stopPoints[index_of_child] + (childwidth / 2);
...
_.cleanClones = function(item) {
  if (k3d.ds.helpers.isCornerItem(item)) {
    ...
    // we need to find the cloned item to remove it... first - if the original
...
_.handleItemViewAction = function(e) {
  ...
  if (target.getActionName() == Actions.CLOSE) {
    ...
    if (newcab.getId() != this.editedChild_.getId()) {
      ...
      // if it is two row item get the relevant current wall dummy and replace
      // it.
      //
      // what to do with two row corner?
...
/**
 * Checks if there is next / previous wall
 * 0 - no wall
 * 1 - wall forward
 * 2 - wall backward
 * 3 - both walls
...
/**
 * Sets the sizes of the wall, this should be done in the controller
...
_.addItem = function(item, idx, is_upper_row) {
  ...
  var w = item.getWidth();
  // On Items that are on the floor there is the kick board (130 mm) and the
  // bench top (50mm); Height is equal to the item's height + the kick board
  ...
  if (is_upper_row) {
    ...
    if (this.currentWallTwos_.length != 0) {
      // try and move the position increasing xoffset until the widths do not
      // intersect
      ...
      for (var i = 0; i < this.currentWallTwos_.length; i++) {
        ...
        // if the two ranges intersect, move the start point to the end of the
